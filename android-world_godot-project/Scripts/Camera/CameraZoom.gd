...
var defalut_motorcycle_zoom = Vector2(.7, .7)
...
func _ready() -> void:
	#EventBus.connect("update_camera_zoom", update_camera_zoom)
	#EventBus.connect("update_camera_zoom_walking", update_camera_zoom_walking)
	#EventBus.connect("update_camera_zoom_bike", update_camera_zoom_bike)
	EventBus.connect("update_motorcycle_moving_speed", update_camera_zoom_from_movement_speed)
...
func update_camera_zoom_bike() -> void:
	var zoom_tween = get_tree().create_tween().set_trans(Tween.TRANS_SINE).set_ease(Tween.EASE_OUT)
	zoom_tween.tween_property(camera, "zoom", defalut_motorcycle_zoom, update_time)
